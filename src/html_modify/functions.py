...
class PCitation:
    ...
    def get_attributes(self,
        ...
        elif is_alpha and citation[0] in 'ivx' and next_value != 1:
            is_rom_numeral = True
        else:
...
#   - next value may be i
#   - is alpha and has two or more characters
# Only runs if the current iteration is an 'i'
def case_i_rnum(curr_iteration,
    ...
    new_list = []
    for x, i in reversed(list(enumerate(curr_list[:(curr_iteration)]))):
...
# Determine whether the number that comes after a Caps paragraph is subset
def case_caps_num(curr_iteration,
                  curr_list
                  ):
    pass
